<?php
    ...
    if(isset($_POST['update_qty_btn'])) {
        ...
        $update_query = mysqli_query($conn, "UPDATE `cart` SET quantity = '$update_value' WHERE id = '$update_qty_id' AND user_id = '$user_id'");
        if ($update_query) {
    ...
    mysqli_query($conn, "DELETE FROM cart WHERE id='$delete_id' AND user_id='$user_id' LIMIT 1") or die('query failed');
...
<body>
    ...
    <?php
        if(isset($_SESSION['success_msg'])){
            ...
            ';
            unset($_SESSION['success_msg']); // ek hi bar dikhe
        }
    ?>
    <!---------about us------------>
    <section class="shop">
        ...
        <div class="box-container"> 
            ...
            <div class="box">
                ...
                <div class="total-amt">
                    Total Amount : 
                    <span>
                        <?php echo $total_amt = ((int)$fetch_cart['price'] * (int)$fetch_cart['quantity']); ?>
                    </span>
                </div>
            </div>
            <?php
